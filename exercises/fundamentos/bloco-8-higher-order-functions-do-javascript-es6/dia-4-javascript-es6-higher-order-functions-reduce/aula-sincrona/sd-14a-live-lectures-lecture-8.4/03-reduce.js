...
let entregasResumo = pedidos.reduce((acc, pedido) => {
    const dadosCerveja = cervejas.find((cerveja)=> cerveja.codigo === pedido.codigoCerveja);
    const entrega = entregas.find((entrega)=>entrega.pedidoId === pedido.id);
    const valorTotal = pedido.quantidade * dadosCerveja.valor

    acc.push({pedido: pedido.id, cerveja: dadosCerveja.descricao, qtd: pedido.quantidade, veiculo: entrega.placaVeiculo, valorTotal: valorTotal})
    return acc
}, [])
